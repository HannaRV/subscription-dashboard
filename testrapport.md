...
- **Språk:** Svenska (påverkar browser native validation messages)
- **Enhet:** Desktop och mobil
- **Plattform:** Windows/macOS

**Validation layers:**

**Frontend (HTML5 native validation):**
- `required` → Browser message: "Fyll i det här fältet"
- `maxlength="20"` → Browser preventerar input över 20 tecken
- `type="number"` + `min="0"` → Browser message: "Välj ett värde som inte är mindre än 0"
- `step="0.01"` → Tillåter decimaler

**Backend (Express middleware validation):**
- SubscriptionValidation middleware validerar all input
- Returnerar 400 Bad Request med JSON error response vid ogiltiga värden

**User experience:**
- Native browser validation (svenska meddelanden)
- Placeholders som inline instruktioner
- Inga custom flash messages (medvetet MVP-val)
...
**Teststeg:**
1. Lägg till prenumeration 1:
   - Name: "Netflix", Price: 139, Frequency: "Monthly", Category: "streaming"
2. Lägg till prenumeration 2:
   - Name: "Spotify", Price: 1200, Frequency: "Yearly", Category: "music"
3. Lägg till prenumeration 3:
   - Name: "SATS", Price: 150, Frequency: "Weekly", Category: "fitness"
...
| **Beskrivning** | Testar att all prenumerationer omräknas korrekt vid byte av visningsfrekvens. |
...
| Aspekt | Beskrivning |
|--------|-------------|
| **Krav** | F1.3.2.2 - Negativt eller ogiltigt pris ska ge felmeddelande |
...
| Aspekt | Beskrivning |
|--------|-------------|
| **Krav** | NF2.2.1 - Hantera rimligt antal prenumerationer |
...
2. Lägg till: Netflix, 169 kr, Monthly, streaming (högre tier)
3. Lägg till: Netflix, 99 kr, Monthly, streaming (lägre tier)
...
**Success Rate:** 100%  
**Kritiska buggar:** 0  
**Kända begränsningar:** Se kravspecifikation sektion 5.1

---

## Kända begränsningar (ej buggar)

Följande begränsningar är dokumenterade i kravspecifikation:

1. **Ingen datapersistens** - Data försvinner vid sidladdning
2. **Ingen edit-funktion** - Måste ta bort och återskapa
3. **Ingen användningsanalys** - UsageAnalyzer från modulen används ej
4. **Ingen kategorianalys** - Ingen gruppering per kategori

Dessa är **medvetna MVP-avgränsningar**, inte buggar.

---

## Slutsats

Subscription Dashboard uppfyller alla funktionella och icke-funktionella krav enligt kravspecifikationen. 

**Validering:**
- ✅ Multi-layer validation (frontend + backend)
- ✅ User-friendly (native browser messages på svenska)
- ✅ Säker (backend skyddar API)

**Funktionalitet:**
- ✅ Add/Remove subscriptions fungerar felfritt
- ✅ Frequency toggle omräknar korrekt
- ✅ Cost calculations matematiskt korrekta
- ✅ Edge cases hanteras väl

**Status:** Godkänd ✅

---

**Testad av:** Hanna Rubio Vretby  
**Testdatum:** 2025-10-17